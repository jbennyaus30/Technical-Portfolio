...
PROCESS(smartfan_process, "HOME: Smart Fan");
AUTOSTART_PROCESSES(&smartfan_process);
...
tcpip_handler(void)
{
  ...
  if(uip_newdata()) {
    ...
    if(strcmp(str, "ON") == 0) {
      ...
      leds_on(LEDS_GREEN);
      leds_off(LEDS_RED);
      printf("HOME: Smart Fan turned ON from server\n");
    } else if(strcmp(str, "OFF") == 0) {
      fan_status = 0;
      leds_off(LEDS_GREEN);
      leds_on(LEDS_RED); // Turn off red LED when light is OFF
      printf("HOME: Smart Fan turned OFF from server\n");
    }
  }
}
/*---------------------------------------------------------------------------*/
...
send_packet(void *ptr)
{
  ...
  sprintf(buf, "Fan is %s", fan_status ? "ON" : "OFF");
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(smartfan_process, ev, data)
{
  PROCESS_BEGIN();
